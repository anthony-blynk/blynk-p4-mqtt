 ...
 */
/* Blynk MQTT client: connects to Blynk Cloud over MQTT (TLS) and implements
 * the Blynk MQTT API - device info on connect, datastream downlinks/uplinks,
 * and the ping/reboot/redirect/OTA control topics.
 *
...
void blynk_mqtt_start(void)
{
    ...
    ESP_LOGI(TAG, "firmware tag @ %p (%u bytes)", (const void *)firmwareTag, (unsigned)sizeof(firmwareTag));
    ...
    ESP_LOGI(TAG, "free heap before connect: %" PRIu32 " bytes", esp_get_free_heap_size());
